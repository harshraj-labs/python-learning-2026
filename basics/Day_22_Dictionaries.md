...
- In Python, dictionaries are built-in data structures that store collections of key-value pairs. They work very similarly to real dictionaries, where we search for a word to find its corresponding meaning.
- With Python dictionaries, we use a key to find its corresponding value. we should use dictionaries when we need to associate values to unique keys. This is helpful when we need to find a value fast based on the key, and when we need to represent structured data.
...
- To update a value, we just need to add the assignment operator, followed by the new value.
- If the key doesn't exist in the dictionary, a new key-value pair will be created.
- The **.get()** method retrieves the value associated with a key. It's similar to the bracket notation that we just used, but its advantage is that we can set a default value, so we won't get an error if the key doesn't exist.
...
- The **.pop()** method removes the key-value pair with the key that we specify as the first argument and returns its value.
